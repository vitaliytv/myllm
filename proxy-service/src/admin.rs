...
//! `/admin/api/stats` + `/admin/api/global-settings`) — talks directly to the
//! upstream server's own admin endpoints, independent of the local reverse
//! proxy port.
//!
...
//! що розвивається сам собою): повертаємо сирий `serde_json::Value`.
//!
//! Headless-адаптація колишнього Tauri `admin.rs`: замість `tauri::State` —
//! `AdminState` (звичайний `Mutex`), замість команд — `pub async fn`,
//! викликані з HTTP-хендлерів `/_proxy/admin/*` (`proxy.rs`).
...
pub enum AdminError {
    ...
    #[error("not connected — call /_proxy/admin/connect first")]
    NotConnected,
}
...
/// Тримає поточну admin-сесію (якщо піднята) — аналог колишнього
/// Tauri-`OmlxState`, тепер звичайний `Mutex` у спільному `AppState`.
#[derive(Default)]
pub struct AdminState(pub Mutex<Option<Session>>);

pub async fn connect(base_url: &str, api_key: &str) -> Result<Session, AdminError> {
...
pub async fn stats(state: &AdminState) -> Result<Value, AdminError> {
    ...
    get_json(&session, "/admin/api/stats").await
}

pub async fn global_settings(state: &AdminState) -> Result<Value, AdminError> {
    let session = current_session(state)?;
    get_json(&session, "/admin/api/global-settings").await
}
...
mod tests {
    ...
    async fn connect_stores_cookie_session() {
        ...
        let session = connect(&server.url(), "secret").await.unwrap();
    ...
    async fn connect_rejects_bad_key() {
        ...
        let err = connect(&server.url(), "wrong").await.unwrap_err();
        match err {
    ...
    async fn get_json_returns_parsed_body() {
        ...
        let session = connect_for_test(&server.url());
        let body = get_json(&session, "/admin/api/stats").await.unwrap();
        assert_eq!(body["total_requests"], 60);
    }

    #[tokio::test]
    async fn stats_without_connect_is_not_connected_error() {
        let state = AdminState::default();
        let err = stats(&state).await.unwrap_err();
        assert!(matches!(err, AdminError::NotConnected));
    }

    fn connect_for_test(base_url: &str) -> Session {
        Session {
