...
> [!IMPORTANT]
> **Worktree-only skill.** Виконується **виключно** в окремому git-worktree (`.worktrees/<current-branch>-adr-normal/`, де slash у назві замінено на `-`) і **не** паралелиться — один інстанс за раз.
...
```bash
mt worktree create "feature-x-adr-normal" --description "n-adr-normal: worktree-only skill"
cd ".worktrees/feature-x-adr-normal"
```

Тобто name для `mt` і шлях для `cd` беруть sanitized форму: slash → `-`; CLI створює власну git-гілку `mt/feature-x-adr-normal`.
